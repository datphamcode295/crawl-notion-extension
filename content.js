...
function startNotionResponseWatcher() {
  ...
  window.addEventListener('message', function messageListener(event) {
    if (event.data.type === 'notionDataCaptured') {
      ...
      // If page is already loaded, process the data
      if (isPageLoaded && !pageInitialized) {
        processContent();
      }
    }
  });
...
async function startPageProcessing() {
  try {
    ...
    console.log(
      'Page fully loaded. Notion response status:',
      notionResponse ? 'Available' : 'Not available yet'
    );
    isPageLoaded = true;
...
async function handlePageChanges(mutations) {
  ...
  if (currentUrl !== lastUrl) {
    ...
    isPageLoaded = false;
    // Don't reset notionResponse as we want to keep watching it
...
function processingBulletList(element) {
  ...
  if (bulletedBlock && bulletedBlock.value.value.properties) {
...
function processingCodeBlock(element) {
  ...
  if (codeBlock && codeBlock.value.value.properties) {
